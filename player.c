...
//recieve potato
	int zyz_flag=1;
	char potato[32];
	printf("client_fd is %d, left_client_fd is %d, right_client_fd is %d\n",client_fd,left_client_fd,right_client_fd);
	printf("host_fd is %d, left_host_fd is %d, right_host_fd is %d\n",host_fd,left_host_fd,right_host_fd);
	...
	while(zyz_flag){
		...
		if(FD_ISSET(client_fd,&set)){
			...
			//server send message, this game is about to end
			char s_message[32];
			memset(s_message,0,sizeof(s_message));
			recv(client_fd,&s_message,sizeof(s_message),0);
			if(strcmp(s_message,"IT")==0){
				//char over[16]="OVER";
				//send(client_fd,over,sizeof(over),0);
				printf("I'm it\n");
				zyz_flag=0;
			}
			else if(strcmp(s_message,"END")==0){
				...
				if(leftorright()==0){
					itostr(next_id,player_info->left_id);
					send(left_client_fd,s_message,sizeof(s_message),0);
					printf("sending potato to %d\n",player_info->left_id);
				}
				else{
					itostr(next_id,player_info->right_id);
					send(right_client_fd,s_message,sizeof(s_message),0);
					printf("sending potato to %d\n",player_info->right_id);
				}
				send(client_fd,next_id,sizeof(next_id),0);//tell server your next move
			}
		}
		else if(FD_ISSET(left_host_fd,&set)){
			recv(left_host_fd,&potato,sizeof(potato),0);
			if(strcmp(potato,"potato")==0){
				char next_id[8];
				memset(next_id,0,sizeof(next_id));
				if(leftorright()==0){
					itostr(next_id,player_info->left_id);
					send(left_client_fd,potato,sizeof(potato),0);
					printf("sending potato to %d\n",player_info->left_id);
				}
				else{
					itostr(next_id,player_info->right_id);
					send(right_client_fd,potato,sizeof(potato),0);
					printf("sending potato to %d\n",player_info->right_id);
				}
				send(client_fd,next_id,sizeof(next_id),0);				
			}	
			...
			recv(right_host_fd,&potato,sizeof(potato),0);
			if(strcmp(potato,"potato")==0){
				char next_id[8];
				memset(next_id,0,sizeof(next_id));
				if(leftorright()==0){
